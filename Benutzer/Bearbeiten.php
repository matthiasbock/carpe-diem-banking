...
if ( $user_id == "new" ) {
	...
	lsit( $password, $real_name, $phone, $email, $inactive ) = mysql_fetch_row( mysql_query("SELECT password,real_name,phone,email,inactive FROM users WHERE user_id='$user_id'") );
	}
echo "</h3></td>
			</tr>
			<tr>
...
function add_transaction()
{
	...
	if ( $sql ) {
		list( $user_id, $username ) = mysql_fetch_row( mysql_query("SELECT user_id,real_name FROM users WHERE real_name='$real_name' AND phone='$phone' AND email='$email'") );
