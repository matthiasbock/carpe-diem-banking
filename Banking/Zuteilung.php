...
page(_($help_context = "Sozialarbeiter an Klient"), false, false, "", $js);
...
if (isset($_GET['added'])) 
{
	...
	$social_new = str_replace(".", ",", urldecode($_GET['socialnew']) );
	$client_new = str_replace(".", ",", urldecode($_GET['clientnew']) );
	...
	<center>
		...
		<font size=4>
		$social hat $client $amount Euro zugeteilt
		</font>
		<br/><br/>
...
<table class='tablestyle2' cellpadding=2 cellspacing=0>
	<tr valign=top><td>
		<table class='tablestyle_inner'>
			...
			<tr>
				<td colspan=2 align=center><h3>Von Sozialarbeiter</h3></td>
			</tr>
			<tr>
				<td class='label'>Sozialarbeiter:</td>
...
echo"					</select>
			...
			<tr>
				<td class='label'>Gutschrift:</td>
				<td>
					<select name='identifier_from' class='combo' title='' autocomplete='off'>\n";
$sql = mysql_query("SELECT transaction_number,amount,`from`,date_from,`to` FROM bank_transactions WHERE `type` = 'GUTSCHRIFT';");
while ( $r = mysql_fetch_row($sql) ) {
	$receiver = mysql_fetch_row( mysql_query("SELECT account_name FROM bank_accounts WHERE account_number = $r[4];") );
	$d = explode("-", $r[3]);
	echo "\t\t\t\t\t\t<option value='$r[0]'>$r[1] Euro an $receiver[0] vom $r[2] am $d[2].$d[1].$d[0]</option>\n";
	}
echo "					</select>
				</td>
			</tr>
...
function add_transaction()
{
	...
	if ( mysql_query( "INSERT INTO bank_transactions (user,type,amount,memo,`from`,identifier_from,date_from,`to`,identifier_to,date_to) VALUES ('$user','ZUTEILUNG','$amount','$memo','$from','$identifier_from','$date_from','$to','$identifier_to','$date_to');" ) ) {
