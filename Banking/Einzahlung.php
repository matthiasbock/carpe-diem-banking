...
page($help_context = "Einzahlung in Kasse", false, false, "", $js);
...
list( $myaccount ) = mysql_fetch_row( mysql_query("SELECT bank_account FROM users WHERE user_id='".$_SESSION['wa_current_user']->user_id."'") );

$account = mysql_fetch_row( mysql_query("SELECT account_name,bank_account_number,bank_code,bank_name FROM `bank_accounts` WHERE `account_type`='SOZIALARBEITER' AND account_number='$myaccount' LIMIT 1;") );
#$account = $result[0];
...
<table class='tablestyle2' cellpadding=2 cellspacing=0>
	<tr valign=top><td>
		<table class='tablestyle_inner'>
			...
			<tr>
				<td class='label'>Abgehoben von:</td>
				<td><i>&nbsp;$account[0]<br/>&nbsp;Konto-Nr. $account[1]<br/>&nbsp;$account[2] $account[3]</i></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input name=identifier_from type=hidden size=9 maxlength=60 value='Barabhebung' type=text><i>&nbsp;Barabhebung</i></td>
			</tr>
			<tr>
				<td colspan=2 align=center><h3>Einzahlung in Kasse</h3></td>
			</tr>
			<tr>
...
echo"				</select>
			...
			<tr>
				<td class='label'>Buchungskennung:</td>
				<td><input name=identifier_to type=hidden size=9 maxlength=60 value='Bareinzahlung von ".$_SESSION["wa_current_user"]->real_name." am ".date("d.m.Y")."' type=text><i>&nbsp;Bareinzahlung von ".$_SESSION["wa_current_user"]->real_name." am ".date("d.m.Y")."</i></td>
			</tr>
			<tr>
...
function add_transaction()
{
	...
	if ( mysql_query("INSERT INTO bank_transactions (user,amount,type,`from`,identifier_from,date_from,`to`,identifier_to,memo) VALUES ('$user','$amount','EINZAHLUNG','$from','$identifier_from','$date_from','$to','$identifier_to','$memo');") ) {
