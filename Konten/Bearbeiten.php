...
include_once($path_to_root . "/includes/shared.php");
...
$readonly = "readonly ";
if ( $account_number == "new" ) {
	echo "<img src='../themes/default/images/new.png' style='vertical-align:middle;' width=48 border='0'/>&nbsp;&nbsp;Neues Klientenkonto anlegen";
	$readonly = "";
	}
else {
	$sql = mysql_fetch_row( mysql_query("SELECT account_type,account_name,birthday,bank_account_number,bank_code,bank_name,cash_account,social_worker FROM bank_accounts WHERE account_number='$account_number' LIMIT 1") );
	$account_type = $sql[0];
	$account_name = $sql[1];
	$birthday = $sql[2];
	$bank_account_number = $sql[3];
	$bank_code = $sql[4];
	$bank_name = $sql[5];
	$cash_account = $sql[6];
	$social_worker = $sql[7];
	echo "Konto bearbeiten: <i>$account_name</i>";
	}
...
if ( $account_number != "new" ) echo "\t\t\t\t&nbsp;<a href='Loeschen.php?account_number=$account_number'>&nbsp;Dieses Konto l&ouml;schen&nbsp;&nbsp;<img src='../themes/default/images/delete.png' border=0 style='vertical-align:middle;' width=48 /></a><br/>\n";
echo "				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input name='birthday' type=text class='textbox' value='$birthday' /></td>
			</tr>
			<tr>
				<td class='label'>Konto-Typ:</td>
				<td>
					<select name='account_type' class='combo'>\n";
if ( $_SESSION['wa_current_user']->user_id == 1 or isset($soz) ) echo "\t\t\t\t\t\t<option value='SOZIALARBEITER'$soz>Sozialarbeiter</option>\n";
if ( $_SESSION['wa_current_user']->user_id == 1 or isset($kas) ) echo "\t\t\t\t\t\t<option value='KASSE'$kas>Kasse</option>\n";
echo "						<option value='KLIENT'$kli>Klient</option>
			...
			<tr>
				...
				<td>
					<select name='cash_account' class='combo'>
						<option value='0'>keine Kasse</option>\n";
...
while ( $r = mysql_fetch_row($sql) ) {
	$selected = "";
	if ( $social_worker == $r[0] or $_SESSION['wa_current_user']->real_name == $r[1] ) $selected = " selected";
	echo "\t\t\t\t\t\t<option value='$r[0]'$selected>$r[1]</option>\n";
	}
echo "					</select>
				</td>
...
else 	echo "<input type=hidden name=social_worker value='$myaccount' />\n";
echo "		</table>
	</td>
	</tr>
</table>
