...
function _expand(tabobj) {
	...
	if (ul.getAttribute("rel")) {
		for (var i = 0; i < alltabs.length; i++) {
			alltabs[i].className = "ajaxbutton"
		}
...
function _set_combo_select(e) {
	...
	e.onblur = function () {
		...
		if ((this.selectedIndex != this.getAttribute('_last')) || ((this.className == 'combo' || this.className == 'combo3') && _update_box(this))) this.onchange();
	}
	e.onchange = function () {
		var s = this;
		this.setAttribute('_last', this.selectedIndex);
		if (s.className == 'combo' || this.className == 'combo3') _update_box(s);
...
function callEditor(key) {
	...
	if (_w.opener == null) _w.opener = self;
	editors._call = key;
	_w.focus();
}
...
function passBack(value) {
	...
	if (value != false) {
		...
		if (to) {
			if (to[0] != undefined) to[0].value = value;
			to.value = value;
...
var inserts = {
	'input': function (e) {
		if (e.onfocus == undefined) {
			e.onfocus = function () {
				save_focus(this);
				if (this.className == 'combo' || this.className == 'combo3') this.select();
			};
		}
	...
	'button.ajaxsubmit,input.ajaxsubmit,input.editbutton,button.editbutton,button.navibutton': function (e) {
		e.onclick = function () {
			...
			if (asp && asp.indexOf('process') !== -1) JsHttpRequest.request(this, null, 60000);
			else JsHttpRequest.request(this);
			return false;
		}
	},
	'button': function (e) {
	...
	'button[aspect=popup]': function (e) {
		...
		e.onclick = function () {
			if (_w) _w.close();
			_w = open(document.location + 'popup=1', "edit", "Scrollbars=0,resizable=0,width=800,height=600, top=50,left=50");
			if (_w.opener == null) _w.opener = self;
			_w.focus();
			return false;
		}
	...
	'select': function (e) {
		if (e.onfocus == undefined) {
			...
			var c = e.className;
			if (c == 'combo' || c == 'combo2' || c == 'combo3') _set_combo_select(e);
		}
	},
			...
			for (var j = 0; j < this.forms.length; j++) {
				...
				for (var i = 0; i < form.elements.length; i++) {
					...
					if (((asp && asp.indexOf('default') !== -1) && key == 13) || ((asp && asp.indexOf('cancel') !== -1) && key == 27)) {
						if (asp.indexOf('process') !== -1) JsHttpRequest.request(el, null, 60000);
						else JsHttpRequest.request(el);
						ev.returnValue = false;
						return false;
					}
				}
